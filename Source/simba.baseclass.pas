...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if NeedHeader then
          DebugLn('The following objects were not freed:');
        NeedHeader := False;
...
begin
  ...
  try
    for I := 0 to TrackedThreads.Count - 1 do
      ...
      begin
        if NeedHeader then
          DebugLn('The following threads were still running:');
        NeedHeader := False;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if NeedHeader then
          DebugLn('The following threads were not freed:');
        NeedHeader := False;
...
begin
  DebugLn('  ' + ClassName + ' (' + HexStr(Self) + ')' + IfThen(Name <> '', ' "' + Name + '"', ''));
end;
...
begin
  DebugLn('  ' + ClassName + ' (' + HexStr(Self) + ')' + IfThen(FName <> '', ' "' + FName + '"', ''));
end;
